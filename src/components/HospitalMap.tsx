...
import {
  ...
  Footprints, ChevronLeft, ChevronRight, RotateCcw, Maximize2, X
} from 'lucide-react';
...
const uiText: Record<string, Record<string, string>> = {
  ar: { title: 'خريطة المبنى التفاعلية', subtitle: 'انقر على أي قسم لعرض اتجاهات المشي', search: 'ابحث عن قسم...', directions: 'اتجاهات المشي', scanQr: 'امسح للخريطة على هاتفك', view360: 'جولة افتراضية', selectDept: 'اختر قسماً من الخريطة', dragHint: 'اسحب الصورة لاستكشاف المبنى' },
  en: { title: 'Interactive Building Map', subtitle: 'Click any department to see walking directions', search: 'Search department...', directions: 'Walking Directions', scanQr: 'Scan for map on your phone', view360: 'Virtual Tour', selectDept: 'Select a department from the map', dragHint: 'Drag the image to explore the building' },
  ku: { title: 'نەخشەی بینای کارلێکەر', subtitle: 'کلیک لەسەر هەر بەشێک بکە بۆ بینینی ئاڕاستەکان', search: 'بگەڕێ بۆ بەشێک...', directions: 'ئاڕاستەی ڕۆیشتن', scanQr: 'سکان بکە بۆ نەخشە لە مۆبایلت', view360: 'گەشتی ڤیرچواڵ', selectDept: 'بەشێک هەڵبژێرە لە نەخشەکە', dragHint: 'وێنەکە ڕابکێشە بۆ گەڕان لە بیناکەدا' },
};
...
const PanoViewer = ({ imageSrc, label }: { imageSrc: string; label: string }) => {
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-48 sm:h-56 rounded-xl overflow-hidden cursor-grab active:cursor-grabbing select-none"
    ...
    >
      ...
      <div className="absolute inset-y-0 start-0 w-12 bg-gradient-to-e from-black/30 to-transparent pointer-events-none" />
      <div className="absolute inset-y-0 end-0 w-12 bg-gradient-to-s from-black/30 to-transparent pointer-events-none" />
      {/* Arrows */}
      <button onClick={() => setOffset(o => o + 120)} className="absolute start-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-background/80 flex items-center justify-center shadow-sm hover:bg-background">
        <ChevronLeft className="w-4 h-4 text-foreground" />
      </button>
      <button onClick={() => setOffset(o => o - 120)} className="absolute end-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-full bg-background/80 flex items-center justify-center shadow-sm hover:bg-background">
        <ChevronRight className="w-4 h-4 text-foreground" />
      </button>
      {/* Reset */}
      <button onClick={() => setOffset(0)} className="absolute top-2 end-2 w-7 h-7 rounded-full bg-background/80 flex items-center justify-center shadow-sm hover:bg-background">
        <RotateCcw className="w-3.5 h-3.5 text-foreground" />
      </button>
      {/* Label */}
      <div className="absolute bottom-2 start-2 bg-background/80 backdrop-blur-sm rounded-lg px-2.5 py-1 text-xs font-medium text-foreground flex items-center gap-1.5">
...
const HospitalMap = () => {
  ...
  const panoramaImg = activeFloor === 0 ? lobbyImg : floor1Img;
  ...
  // QR URL points to the current page with map section hash
          ...
          <div className="bg-card rounded-2xl shadow-elevated overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row">
              ...
              <div className="flex-1 p-4">
                ...
                {activeTab === '360' && (
                  <div className="mb-4">
                    <PanoViewer imageSrc={panoramaImg} label={`${txt.view360} — ${floors[activeFloor]}`} />
                    <p className="text-xs text-muted-foreground mt-2 text-center">{txt.dragHint}</p>
                  </div>
                )}
                          ...
                          >
                            ...
                            >
                              <div className="flex items-center justify-center w-full h-full">
                                <Icon className="w-4 h-4" style={{ color: isActive ? section.color : section.color }} />
                              </div>
                            </foreignObject>
                            <text
